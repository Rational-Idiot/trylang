#![allow(dead_code)]
use crate::env::Env;
use crate::expr::{binding_usage::BindingUsage, block::Block};
...
impl Number {
    fn new(s: &str) -> Result<(&str, Self), String> {
        let (s, num) = utils::extract_digits(s)?;
        Ok((
            s,
            Number(num.parse().expect("Cannot parse the given val to an i32")),
        ))
    }
}
...
impl Expr {
    ...
    fn new_num(s: &str) -> Result<(&str, Self), String> {
        Number::new(s).map(|(s, num)| (s, Expr::Number(num)))
    }
    ...
    pub(crate) fn eval(&self, env: &mut Env) -> Result<Val, String> {
        match self {
            ...
            Self::BindingUsage(bind) => bind.eval(env),
...
#[cfg(test)]
mod tests {
    use crate::{expr::block::Block, statement::Statement};
